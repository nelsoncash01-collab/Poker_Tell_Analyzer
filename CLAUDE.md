...
- **Repository scaffolding + PTS-based video ingestion + JSON/CSV hand-history ingestion + a `poker-tell` CLI + sync layer foundation** is in place (`src/poker_tell/`):
  ...
  - `cli.py` — `poker-tell` CLI (entry point in `pyproject.toml`; also `python -m poker_tell.cli`): `download` (yt-dlp), `ingest-video` (single or `--catalog` batch), `ingest-hands` (JSON/CSV), `ls`, and `read-frame` / `dump-regions` (broadcast OCR). Expected user errors (bad input, leakage guard, missing file, missing optional dep) print cleanly and return exit code 1.
  - `broadcast/` — Stage 1 of OCR-based hand reconstruction from on-screen graphics: `layout.py` (pillarbox-aware overlay regions as fractions of the active area; `POKERGO_CLASSIC_HSP` first-pass calibration), `cards.py` (numpy-only rank+suit card-tile recognizer via color + template matching), `ocr.py` (pure `parse_pot`/`parse_status` parsers + `read_frame` orchestrator; text step lazily imports pytesseract+Pillow). Reads game-state ground truth (pot/board/cards/actions) — not behavioral content, so it sits outside the no-transfer constraint. Stages 2–4 (auto hand segmentation, assembling `HandHistory` + `SyncTable`, bluff/value labeling) are not yet built.
