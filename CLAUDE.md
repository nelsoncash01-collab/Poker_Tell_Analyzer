...
- **Repository scaffolding + PTS-based video ingestion + JSON/CSV hand-history ingestion + a `poker-tell` CLI + sync layer foundation** is in place (`src/poker_tell/`):
  ...
  - `cli.py` — `poker-tell` CLI (entry point in `pyproject.toml`; also `python -m poker_tell.cli`): `download` (yt-dlp), `ingest-video` (single or `--catalog` batch), `ingest-hands` (JSON/CSV), `ls`, `read-frame` / `detect-overlays` / `format-segments` (broadcast OCR). Expected user errors (bad input, leakage guard, missing file, missing optional dep) print cleanly and return exit code 1.
  - `broadcast/` — Stage 1 of OCR-based hand reconstruction from on-screen graphics. Overlays are **detected by appearance every frame** (not fixed coordinates) because the compilation splices clips at different crops/zooms: `detect.py` (OpenCV — maroon POT/name chrome by colour + white card tiles by shape; the POT banner anchors + scales the rest), `formats.py` (learn the POT banner's colour/location and detect format changes — `track_formats`/`segment_profiles`; banner found colour-agnostically via its `$#,###` text), `cards.py` (numpy-only rank+suit card-tile recognizer), `ocr.py` (pure `parse_pot`/`parse_status` parsers + `read_frame`, which now defaults to detection; text step lazily imports pytesseract+Pillow). `layout.py` (fixed fractional ROIs) is retained only as a manual fallback. Reads game-state ground truth (pot/board/cards/actions) — not behavioral content, so it sits outside the no-transfer constraint. Stages 2–4 (auto hand segmentation, assembling `HandHistory` + `SyncTable`, bluff/value labeling) are not yet built.
